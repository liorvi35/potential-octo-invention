...
const clearBoardGridLayoutOverride = () => {
  boardGrid.style.display = "";
  boardGrid.style.placeItems = "";
  boardGrid.style.gridTemplateColumns = "";
  boardGrid.style.aspectRatio = "";
  boardGrid.style.maxWidth = "";
  boardGrid.style.margin = "";
};

const resetResult = () => {
  boardGrid.innerHTML = "";
  clearBoardGridLayoutOverride();
...
const renderBoardImage = (boardImage, boardImageMimeType = "image/jpeg") => {
  if (typeof boardImage !== "string" || !boardImage.startsWith("data:image/")) {
    throw new Error("Backend response is missing the rendered board image.");
  }

  boardGrid.innerHTML = "";
  boardGrid.style.display = "grid";
  boardGrid.style.placeItems = "center";
  boardGrid.style.gridTemplateColumns = "1fr";
  boardGrid.style.aspectRatio = "1 / 1";
  boardGrid.style.maxWidth = "520px";
  boardGrid.style.margin = "0 auto";

  const image = document.createElement("img");
  image.src = boardImage;
  image.alt = "Rendered chess board";
  image.title = `Rendered chess board (${boardImageMimeType})`;
...
const renderResponse = (data) => {
  ...
  if (data.boardImage) {
    renderBoardImage(data.boardImage, data.boardImageMimeType);
  } else {
    renderBoard(board, data.unicodeBoard, data.nameBoard || data.pieceNameBoard);
  }
